...
import { Boxes, ChevronDown, Target, Zap, TrendingUp, Layout, MousePointer2, ArrowRight } from "lucide-vue-next";
...
<template>
  ...
  >
    <div class="flex gap-8 items-center">
      <router-link to="/" @click="closeMenu" class="text-xl font-black tracking-tighter">Simetrik</router-link>
        ...
        <div class="relative">
          <button
            ...
            :class="[
              ...
              isOpen ? 'bg-zinc-100 text-black' : 'text-zinc-500 hover:text-black hover:bg-zinc-50'
            ]"
          >
            Servicios
            <ChevronDown :size="14" :class="['transition-transform duration-300', { 'rotate-180': isOpen }]" />
          </button>
          ...
          >
            ...
            >
              <div class="flex gap-12">
                <!-- Left Side: Context -->
                <div class="w-1/3">
                  <h3 class="text-lg font-bold tracking-tight mb-3">Especialidades</h3>
                  <p class="text-xs text-zinc-400 leading-relaxed mb-6">
                    Explora cómo nuestras áreas se integran para escalar tu negocio.
                  </p>
                  <router-link 
                    to="/pricing" 
                    @click="closeMenu"
                ...
                <div class="w-2/3 grid grid-cols-1 gap-2">
                  ...
                  >
                    <div class="p-2.5 rounded-xl bg-zinc-100 group-hover:bg-white group-hover:shadow-sm transition-all text-zinc-900">
                      <component :is="service.icon" :size="20" />
                    </div>
                    <div>
                      <h4 class="text-sm font-bold text-zinc-900 mb-0.5">{{ service.title }}</h4>
                      <p class="text-[11px] text-zinc-400 group-hover:text-zinc-600 transition-colors">{{ service.desc }}</p>
                    </div>
                  </router-link>
                </div>
              </div>
              ...
              <!-- Bottom Accent -->
              <div class="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 to-pink-500 opacity-20"></div>
            </div>
          </transition>
        </div>

        <div class="text-zinc-500 hover:text-black cursor-pointer transition-colors font-medium text-sm">
          Contacto
        </div>
      </div>
    </div>
